...
export const useChainStore = defineStore('chain', {
  ...
  actions: {
    ...
    async fetchChainData(chain) {
      ...
      const mcap =
        this.ethPrice !== '$--'
          ? `$${((totalSupply * parseFloat(this.ethPrice.replace(/[^0-9.]/g, ''))) / 1e9).toFixed(
              2
            )}B`
          : '--';
      ...
      return {
        block: bNum,
        blockFmt: bNum.toLocaleString(),
        blockSub: `${txs} txs · ${utilPct.toFixed(1)}% gas · ${new Date(ts * 1000).toLocaleTimeString()}`,
        gas: priorityFee,
        gasFmt: priorityFee.toFixed(3),
        baseFee,
        baseFeeFmt: baseFee.toFixed(3),
        blobFee,
        blobFeeFmt: blobFee !== null ? blobFee.toFixed(4) : 'N/A',
        utilPct,
        utilPctFmt: utilPct.toFixed(1),
    ...
    async loadHistory(chainId = null, days = 7) {
      try {
        ...
        logger.warn('Failed to load history from API, using localStorage:', {
          error: error.message,
        });
    ...
    async fetchMEVData(chain) {
      try {
        const response = await fetch(
          `https://api.flashbots.net/v2/blocks?limit=1&chain_id=${chain.id === 'ethereum' ? '1' : chain.id === 'arbitrum' ? '42161' : '10'}`
        );
