...
apiClient.interceptors.request.use(
  (config) => {
    ...
    pendingRequests.set(
      requestKey,
      new Promise((resolve) => {
        (config as unknown as { __customResolve?: (value: unknown) => void }).__customResolve =
          resolve;
      })
    );
...
apiClient.interceptors.response.use(
  (response) => {
    ...
    const customResolve = (
      response.config as unknown as { __customResolve?: (value: unknown) => void }
    ).__customResolve;
    if (customResolve) {
