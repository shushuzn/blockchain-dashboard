...
export function usePreloader() {
  ...
  const preloadMultiple = async (
    ...
    return Promise.allSettled(
      ...
      return results.map((r) => r.status === 'fulfilled' ? r.value : undefined as unknown as void)
    })
  }
  ...
  const preloadCriticalResources = async () => {
    const criticalResources: Array<{ url: string; options: PreloadOptions }> = [
      { url: 'https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap', options: { as: 'style' } },
  ...
  const preloadFonts = async (fonts: Array<{ family: string; weights?: string[] }>): Promise<void> => {
    const fontUrls = fonts.flatMap(({ family, weights }) => {
      const weightsList = weights || ['400', '500', '600', '700']
      return weightsList.map((weight) => 
