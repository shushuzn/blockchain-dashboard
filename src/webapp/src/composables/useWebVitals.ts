...
function sendToAnalytics({ name, delta, id, rating }: { name: string; delta: number; id: string; rating: string }) {
  ...
  if (import.meta.env.VITE_ANALYTICS_ENDPOINT) {
    ...
    }).catch((error: Error) => logger.error('[Web Vitals] Failed to send to analytics:', { error: error.message }))
  }
}
...
function getOverallScore(): number {
  ...
  if (summary.CLS) {
    scores.push(summary.CLS.rating === 'good' ? 100 : summary.CLS.rating === 'needs-improvement' ? 50 : 0)
  }
  if (summary.LCP) {
    scores.push(summary.LCP.rating === 'good' ? 100 : summary.LCP.rating === 'needs-improvement' ? 50 : 0)
  }
  if (summary.FID !== undefined || summary.INP) {
    scores.push((summary.FID || summary.INP)?.rating === 'good' ? 100 : (summary.FID || summary.INP)?.rating === 'needs-improvement' ? 50 : 0)
  }
